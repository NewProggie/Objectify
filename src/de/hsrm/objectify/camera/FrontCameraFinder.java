	...
	public Camera open(Context context) {
		...
		for (int i = 0; i < Camera.getNumberOfCameras(); i++) {
			...
			if (cameraInfo.facing == Camera.CameraInfo.CAMERA_FACING_FRONT) {
				...
				SharedPreferences preferences = SettingsActivity
						.getSettings((ContextWrapper) context);
				String userSettingPicDim = preferences
						.getString(
								context.getString(R.string.settings_camera_resolutions),
								"");
				Size picDim;
				if (userSettingPicDim == "") {
					picDim = new Size(
							params.getSupportedPictureSizes().get(0).width,
							params.getSupportedPictureSizes().get(0).height);
				} else {
					String[] dims = userSettingPicDim.split("x");
					picDim = new de.hsrm.objectify.utils.Size(
							Integer.valueOf(dims[0]), Integer.valueOf(dims[1]));
				}
