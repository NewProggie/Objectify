...
public class MathHelper {
	 ...
	 * @return
	 */
	public static double[][] twoDimIntegration(Matrix pGradients,
			Matrix qGradients, int imageHeight, int imageWidth) {
		...
		for (int i = 0; i < imageHeight; i++) {
			for (int j = 0; j < imageWidth; j++) {
				if ((i != 0) || (j != 0)) {
					...
					double uv = u * u + v * v;
					double d = uv;
					zComplex[i][idx] = (u * pComplex[i][idx + 1] + v
							* qComplex[i][idx + 1])
							/ d;
					zComplex[i][idx + 1] = (-u * pComplex[i][idx] - v
							* qComplex[i][idx])
							/ d;
				}
				idx += 2;
